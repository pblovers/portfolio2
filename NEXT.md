...
> 호버·스크롤과 무관한 기존 `fitWordmark()`(HANDOFF 5절) 문제.~~
> → **2026-08-05 해결.** `fitWordmark()` 문제가 아니었다. 태블릿 고정 푸터 박스를
> 뷰포트 **위**(`inset:0`+`height:60vh`)에 붙여 둔 게 원인 — 원본은 **아래**(bottom:0)에
> 붙는다. 바로잡으니 워드마크가 저절로 811 로 맞았다. 아래 6절 참고.
...
# 푸터 기하 (픽셀 대조로는 '아래 여백'이 어두운색끼리라 TOL 에 묻힌다 — 6절 교훈)
node footfix.mjs index works -- 1024x900 1024x700   # 원본의 fixed 푸터 박스 rect·내부 위치
node footgeo.mjs index 1024x900                     # 원본 vs 구현 docH·푸터 rect 한눈에
node footmine.mjs works.html -- 1024x900 810x700    # 구현만 여러 뷰포트로 (바닥을 칠하는 요소까지)
node footcol.mjs index 1024x900                     # 세로 색 훑기 — 배경 띠(이음매) 탐지
...
## 6. 태블릿 고정 푸터 — 2026-08-05 수정 (푸터 잘림/아래 여백)

증상: 태블릿 폭(810~1279)에서 스크롤 끝에 푸터가 뷰포트 위쪽 절반만 차지하고
그 아래로 body 배경 띠가 남았다. **모든 페이지**에서 났다.

원인 2가지 (둘 다 고쳐야 한다):

1. **푸터 박스가 뷰포트 위에 붙어 있었다.** `.footer` 는 `position:fixed; inset:0`
   인데 태블릿에서 `height:60vh` 만 덮어써서 top:0 기준 60vh 가 됐다.
   원본은 **bottom:0 기준**이다 (실측 1024x900 index 360~900 / works 450~900).
2. **리빌 스페이서가 푸터 박스보다 컸다.** works(100vh)·상세(100vh)가 그랬다.
   스페이서는 반드시 푸터 박스 높이와 같아야 흰 콘텐츠 하단이 푸터 상단에 정확히
   맞물린다.

원본 실측 구조 (전부 `tools/footfix.mjs` 로 fixed 컨테이너 rect 를 직접 읽었다):

| | index | 그 외 전부 |
|---|---|---|
| 푸터 박스 높이 | 60vh | 50vh |
| 위치 | `top: 100vh - 높이`, bottom 0 | 〃 |

박스가 낮아도 콘텐츠(링크·워드마크·카피라이트)는 안 줄고 **아래로 넘친다**
(원본도 1024x900 works 에서 카피라이트가 화면 26px 밖). 최소 높이는 상수가 아니라
폭 비례 = `7.89vw + 411.6px` (워드마크만 폭 따라 커지기 때문. 실측 810→475 /
1024→492 / 1279→512, 워드마크 높이 빼면 416.5 로 일정).

구현: `--foot-h`(:root 50vh, `.home-page` 60vh) 하나로 푸터 박스·키링·스페이서를 묶었다.
`css/common.css` 태블릿 블록 + 각 페이지 스페이서(`.page-spacer`/`.wsec-spacer`/
`.msec-spacer`/`.wd-spacer`) 참고.

검증: 폭 3종(810/1024/1279) x 높이 3종(700/900/1024) x 7페이지, 원본과 전부 1px 이내.
`node tools/footmine.mjs <파일> -- 1024x900 810x700` / `node tools/footfix.mjs index works -- 1024x900`.

같이 고친 것:
- **상세 페이지 푸터 링크가 안 보였다** — `.wd-page` 가 body 에 `color:#1a1a1a` 를
  거는데 푸터 배경도 #1a1a1a 다. `.footer-links` 에 색을 못박았다 (모든 폭에서 났던 버그).
- **모바일 services 글자** — 원본은 28/36 인데 32/40 으로 키워 놓아 목록이 8px 위에
  붙어 있었다. 기본값으로 되돌리고 row-gap 4→8 (피치 44 유지).
