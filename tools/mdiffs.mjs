...
const SY = Number(process.argv[4] || 0);
...
for (const [tag, url] of [
  ...
  const file = `./diff/m-${tag}-${W}-${SY}.png`;
...
writeFileSync(`./diff/m-diff-${W}-${SY}.png`, PNG.sync.write(out));
...
console.log(`\ndiff 이미지: ./diff/m-diff-${W}-${SY}.png (빨강 = 다른 픽셀)`);
